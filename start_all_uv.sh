...
# 启动招聘系统的所有服务 - UV版本
# 用法:
#   ./start_all_uv.sh           # 后台运行（默认）
#   ./start_all_uv.sh -f        # 前台运行显示日志
#   ./start_all_uv.sh stop      # 停止所有服务
#   ./start_all_uv.sh status    # 查看服务状态
#   ./start_all_uv.sh restart   # 重启所有服务
...
stop_services() {
    ...
    pkill -f "uv run python admin_app.py" 2>/dev/null || true
...
start_background() {
    ...
    nohup uv run python app.py > "$LOG_FILE_PUBLIC" 2>&1 &
    ...
    nohup uv run python admin_app.py > "$LOG_FILE_ADMIN" 2>&1 &
    echo $! > "$PID_FILE_ADMIN"
    ADMIN_PID=$!
